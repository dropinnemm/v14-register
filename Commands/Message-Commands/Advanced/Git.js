...
module.exports = {
    ...
    async run({ client, message, args }) {
        ...
        if (message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            ...
            Odaya giden kullanıcı : ${message.member} - \`${message.member.user.id}\`
            Odasına Gidilen Kişi : ${üye} - \`${üye.id}\`
            Kanal : <#${üye.voice.channelId}>
            `)
            ...
            .setAuthor({name : message.member.user.username , iconURL :  message.member.user.displayAvatarURL({dynamic : true , size : 4096})})
            .setDescription(`
             ${message.member} kullanıcısı senin yanına gelmek istiyor onaylıyor musun?,
